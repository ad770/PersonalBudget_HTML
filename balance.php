...
<body>
            ...
            <div class="row">
                <div class="col-10 col-sm-10 col-md-8 col-lg-7 mx-auto">
                    <ul class="nav nav-tabs nav-justified text-center">
                        ...
                        <li class="nav-item active col-12 col-sm-3">
                            <a class="nav-link disabled" href="balance.php"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-shuffle" viewBox="0 0 15 15">
                    ...
                    <div id="wrapper" class="show_balance">
                        <div class="row text-center col-3 mx-auto">
                            ...
                            <h3 <?php if ($month_balance < 0) {
                                    echo 'class= "text-center text-danger"';
                                } else {
    ...
    <script type="text/javascript">
        ...
        google.charts.setOnLoadCallback(draw_income_chart);
        google.charts.setOnLoadCallback(draw_expense_chart);
        ...
        function draw_income_chart() {
            let income_data = google.visualization.arrayToDataTable([
                ...
                while ($income_to_chart_result = $get_income_data_to_chart->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $get_income_cat_to_chart->execute();
                    $income_cat_result_to_chart = $get_income_cat_to_chart->fetch(PDO::FETCH_ASSOC);
                    echo ("[" . "'" . $income_cat_result_to_chart['name'] . "', " . $income_to_chart_result['income_sum_amount'] . ", '" . $income_to_chart_result['income_sum_amount'] . "'],");
                }
                ?>
            ]);
            ...
            let income_chart = new google.visualization.BarChart(document.getElementById('income_chart'));
            income_chart.draw(income_data, income_options);
        }
        ...
        function draw_expense_chart() {
            let expense_data = google.visualization.arrayToDataTable([
                ...
                <?php
                $get_expense_data_to_chart = $db->prepare("SELECT SUM(amount) AS expense_sum_amount, expense_category_assigned_to_user_id AS expense_cat_id FROM expenses WHERE user_id='$user_id' and date_of_expense between '$begin_date' and '$end_date' GROUP BY expense_category_assigned_to_user_id");
                $get_expense_data_to_chart->execute();

                while ($expense_to_chart_result = $get_expense_data_to_chart->fetch(PDO::FETCH_ASSOC)) {
                    $expense_category_id_to_chart = $expense_to_chart_result['expense_cat_id'];
                    $get_expense_cat_to_chart = $db->prepare("SELECT name FROM expenses_category_assigned_to_users WHERE id=$expense_category_id_to_chart");
                    $get_expense_cat_to_chart->execute();
                    $expense_cat_result_to_chart = $get_expense_cat_to_chart->fetch(PDO::FETCH_ASSOC);
                    echo ("[" . "'" . $expense_cat_result_to_chart['name'] . "', " . $expense_to_chart_result['expense_sum_amount'] . ", '" . $expense_to_chart_result['expense_sum_amount'] . "'],");
                }
                ?>
            ]);
            ...
            let expense_chart = new google.visualization.BarChart(document.getElementById('expense_chart'));
            expense_chart.draw(expense_data, expense_options);
        }
    </script>
    <script>
